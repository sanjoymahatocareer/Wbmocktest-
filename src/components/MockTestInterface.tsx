...
export default function MockTestInterface({
  ...
  const [visitedQuestions, setVisitedQuestions] = useState<Record<string, boolean>>({ [test.questions[0].id]: true });
  ...
  return (
    <div className="space-y-4 font-sans max-w-md mx-auto">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-4 shadow-sm space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="font-bold font-sans">প্রশ্ন: {currentQuestionIndex + 1} / {test.questions.length}</span>
          </div>

          {/* TIMER */}
          <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black shadow-inner border ${
            timeLeftSeconds < 60 
      ...
      {/* 2. Questions Area */}
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4 relative">
        <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/40 px-3 py-1.5 rounded-xl text-xs font-bold text-slate-500">
          ...
          <span className="text-emerald-600 dark:text-emerald-400">মান: +১.০ | নেগেটিভ: -০.২৫</span>
        </div>
          ...
          {currentQuestion.options.map((option, idx) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 p-3.5 rounded-2xl border text-left text-xs font-bold transition-all ${
                  isSelected
      ...
      {/* 3. Review Box & Question Palette Panel */}
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-4 shadow-sm space-y-4">
        ...
        {/* Guide labels */}
        <div className="grid grid-cols-4 gap-1 pt-2 border-t border-slate-50 dark:border-slate-800 text-[9px] font-bold text-slate-400">
      ...
      {/* 4. Bottom Controls Toolbar */}
      <div className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 rounded-3xl p-4 shadow-sm flex items-center justify-between gap-2">
        <button
          onClick={handlePrev}
          disabled={currentQuestionIndex === 0}
          className="p-3 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 rounded-2xl active:scale-95 disabled:opacity-40 font-bold text-xs flex items-center gap-1 transition-all"
        >
        ...
        <button
          ...
          className={`px-3 py-3 rounded-2xl text-[11px] font-extrabold flex items-center justify-center gap-1.5 transition-all active:scale-95 border ${
        ...
        {currentQuestionIndex === test.questions.length - 1 ? (
          <button
            ...
            className="px-4 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-extrabold rounded-2xl active:scale-95 text-xs flex items-center gap-1 transition-all"
          >
          ...
          <button
            onClick={handleNext}
            className="px-4 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-extrabold rounded-2xl active:scale-95 text-xs flex items-center gap-1 transition-all"
          >
